...
(function(){var
F=fdate.getFormatter=function(f,utc){var x=fdate.entero(0,"fdate.getFormatter",arguments),a=[],c,n,i,t,r=F.r,R=0,rR=0;
for(i=0;i<f.length;i+=n){
c=f.charAt(i);
for(n=1;i+n<f.length&&f.charAt(i+n)===c;n++);
c=f.substring(i,i+n);
if(c==='x'){
rR=R=F.fi[f.charAt(i+n)];
R="(R."+R[0]+"===D."+R[0]+")&&Math.abs(D-R)<"+R[1];
}else{
if(utc&&F.f["UTC"+c])c="UTC"+c;
if(F.f[c]){
t=utc&&c.substring(0,2)!="UTC"?F.f[c].replace(r,"D.getUTC"):F.f[c];
a.push(R?"("+R+"?'':"+t+")":t);
}else if((t=a[a.length-1]).charAt(0)==="'"&&t.substring(t.length-1)==="'")a[a.length-1]=t.substring(0,t.length-1)+c+"'";else a.push("'"+c+"'");
R=0;
}}
c=new Function("o","var "+(fdate.consolentero?"x=console.entero('fdate.getFormatter(\""+f.replace(/"/g,'\\"')+"\","+utc+")',arguments.callee,arguments),":"")+"D=typeof o==='undefined'?new Date:o instanceof Date?o:new Date(o)"+(rR?",R=new Date":"")+";return "+(fdate.consolentero?"x(":"")+"isNaN(D)?'Invalid date: '+o:"+a.join("+")+(fdate.consolentero?")":"")+";");
x();return c;};
F.r=/D\.get/g;
F.fi={
...
uu:"(D.getMilliseconds()+1000+'').substring(1,3)",
uuu:"(D.getMilliseconds()+1000+'').substring(1,4)",
